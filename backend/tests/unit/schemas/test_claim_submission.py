...
def should_accept_a_complete_certified_payload() -> None:
    ...
    request = ClaimSubmissionRequest.model_validate(valid_payload())
...
def should_reject_a_non_positive_or_over_precise_total(bad_total: str) -> None:
    ...
    with pytest.raises(ValidationError):
        ClaimSubmissionRequest.model_validate(valid_payload(total_amount=bad_total))
